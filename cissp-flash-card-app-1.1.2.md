# CodeLens: cissp-flash-card-app-1.1.2
...
- **Project:** cissp-flash-card-app-1.1.2
- **Files Analyzed:** 21
- **Total Lines:** 7,611
- **JS Files:** 13
- **HTML Files:** 3
...
js\importer.js (319 lines)
js\parsers.js (528 lines)
js\popup.js (324 lines)
...
- **Lines:** 332
- **Functions (8):**
  - `navigateTo(viewName)` (line 38)
  - `async initHome()` (line 74)
  - `initImportView()` (line 112)
  - `async handleFile(file)` (line 157)
  - `showImportPreview(result)` (line 186)
  - `async confirmImport()` (line 280)
  - `cancelImport()` (line 307)
  - `getInitialView()` (line 314)
...
- **Functions (31):**
  ...
  - `async saveCard(card)` (line 172)
  - `async saveCards(cards)` (line 177)
  - `prepareCardForStorage(card)` (line 189)
  - `async deleteCard(id)` (line 216)
  - `async getCardsByDomain(domain)` (line 217)
  - `async getCardCount()` (line 218)
  - `async getCardsDueByDate(dateStr)` (line 223)
  - `async getAllDecks()` (line 251)
  - `async saveDeck(deck)` (line 252)
  - `async deleteDeck(id)` (line 253)
  - `async getAllSessions()` (line 258)
  - `async saveSession(session)` (line 259)
  - `async getSetting(key)` (line 264)
  - `async setSetting(key, value)` (line 269)
  - `async buildTermIndex()` (line 276)
  - `async resolveRelatedCard(termStr, termIndex)` (line 287)
  - `async migrateCards()` (line 297)
  - `DB(() =>` (line 181)
...
- **Functions (8):**
  ...
  - `_contentKey(card)` (line 208)
  - `async confirmImport()` (line 214)
  - `cancelImport()` (line 278)
  - `getPending()` (line 282)
  - `updateDuplicateAction(index, action)` (line 286)
  - `extractContributor(newCards, duplicates)` (line 292)
  - `Importer(() =>` (line 198)
...
- **Lines:** 528
- **Functions (9):**
  - `generateImportId(originalId, fileName)` (line 13)
  - `parse(content, fileName)` (line 21)
  - `parseJSON(content, fileName)` (line 44)
  - `normalizeJSONItem(item, fileName, index)` (line 108)
  - `parseCSV(content, fileName)` (line 229)
  - `parsePlainText(content, fileName)` (line 330)
  - `matchFieldLabel(line)` (line 440)
  - `parseCSVLines(text)` (line 466)
  - `Parsers(() =>` (line 194)
...
- **Lines:** 324
- **Functions (9):**
  - `openStudyDeck(hash)` (line 26)
  - `openDB()` (line 70)
  - `getAllFromStore(db, storeName)` (line 110)
  - `getSettingFromStore(db, key)` (line 124)
  - `todayStr()` (line 141)
  - `getMasteryTier(card)` (line 145)
  - `isDueToday(card)` (line 160)
  - `safeSetHTML(id, html)` (line 170)
  - `safeAddClick(id, handler)` (line 176)
- **Constants:** `DB_NAME, DB_VERSION, DOMAINS`
- **Event Listeners:** `click`
- **DOM Queries:** `popupThemeToggle, popupBody`
